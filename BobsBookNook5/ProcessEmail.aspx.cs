...
public partial class ProcessEmail : System.Web.UI.Page
{
            ...
            if (File.Exists(fileName))
                using (StreamReader sr = new StreamReader(fileName))
                {
                    while (sr.Peek() >= 0)
                    {
                        ...
                        if (lineIn.StartsWith("<TITLE>") || lineIn.StartsWith("<AUTHOR>")
                       || lineIn.StartsWith("<IMAGE>"))
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        ...
        while (lstRawXML.Items.Count > 0)
        {
            ...
            sqlCommand = "SELECT DISTINCT " + txtOwnerID.Text + "HISTORY.EMAIL FROM " +
            txtOwnerID.Text + "HISTORY WHERE " + txtOwnerID.Text + "HISTORY.ISBN IN (SELECT DISTINCT " + txtOwnerID.Text + "BOOKS.ISBN FROM " + txtOwnerID.Text + "BOOKS WHERE " + txtOwnerID.Text + "BOOKS.AUTHOR = '" + author + "')";
            fillListBox(sqlCommand, lstEmail, txtOwnerID.Text + "HISTORY", "EMAIL");
    ...
    void fillListBox(string sqlCommand, ListBox listBox, string tableName, string field)
    {
        DataRow dr;
        ...
        { //open the connection
            ...
            myDatabaseConnection.myConnection);
            da.Fill(dt);
