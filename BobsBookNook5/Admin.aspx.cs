...
public partial class Admin : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //using Trim() function makes sure return and line feed codes are removed
        ...
        checkTables(txtOwnerID.Text + "BOOKS", "CREATE TABLE " + txtOwnerID.Text + "BOOKS (Title CHAR(64), ISBN CHAR(14) UNIQUE, Author CHAR(64), Category CHAR(64), Image CHAR(128), Description CHAR(255))");

        checkTables(txtOwnerID.Text + "BESTSELLER", "CREATE TABLE " + txtOwnerID.Text + "BESTSELLER (ISBN CHAR(20))");

        checkTables(txtOwnerID.Text + "HISTORY", "CREATE TABLE " + txtOwnerID.Text + "HISTORY (EMAIL CHAR(64), ISBN CHAR(14))");
    ...
    protected void btnLoadImages_Click(object sender, EventArgs e)
    {
        ...
        if(!FileUpload1.HasFiles)
        {
            ...
            lstProgress.Items.Add("all images from te directory.");
            FileUpload1.Focus();
        }
        else
        {
    ...
    protected void btnLoadXML_Click(object sender, EventArgs e)
    {
        ...
        {
            //if not, error messageis displayed and focus set to fileupload
    ...
    private void decode(ListBox lstHidden)
    {
        ...
        //we do not need the <?xml version="1.0" encoding="utf"?
        ...
        while (lstHidden.Items.Count > 1)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                //We are just saving the folder and filename
                if (i == 4)
                {
                ...
                {
                    //if description is over 255 characters, it needs to be truncated to fit our field descriptiors
                    if (i == 5) //255 length
                    {
                        if (field.Length >= 255)
                        {
                ...
                if (i < 5) //5 is the last field, we are not there yet so add a ,
                {
                    sqlStatement += ", ";
                }
    ...
    protected void btnToBookstore_Click(object sender, EventArgs e)
    {
    ...
    protected void btnRemoveBookTitlesfromCategory_Click(object sender, EventArgs e)
    {
